...
export class PixiJSTable extends PIXI.Container {
  ...
  constructor(debugMode = false,
    ...
    this.title = tableTitle; // the title of the table. Optional. Null will not draw the title.
  ...
  deleteRow(rowNumber) {
    ...
    if (rowNumber > -1 && rowNumber < this.rowCount) {
      ...
      throw new Error(`Unable to delete row ${rowNumber}. Row does not exist.`);
    }
    this.updateRows();
  }
  ...
  addCell(displayObject, rowNumber = this.rowCount - 1) {
    ...
    if (this.rowCount === 0) {
      throw new Error('Can\'t add cell when there are no rows!');
    } else {
      const row = this.rows[rowNumber];
  ...
  deleteCell(rowNumber, cellNumber) {
    ...
    if (rowNumber > -1 && rowNumber < this.rowCount) {
      ...
      throw new Error(`Unable to delete cell (${rowNumber}, ${cellNumber}). Row does not exist.`);
    }
    this.updateRows();
  }
  ...
  updateRows() {
    ...
    let currentColumn = 0;
    let maxColWidth = 0;
    const columnSeparation = [this.cellStartPosition.x];
    const _inst = this;
    const rowSeparation = [this.rowStartPosition.y + this.title.height];
    ...
    while (currentColumn < this.maxCols) {
      this.rows.forEach((row, index) => {
        if (_inst.getCell(index, currentColumn)) {
          const cell = _inst.getCell(index, currentColumn);
   ...
   */
  debugLog(method: string, value: any = null) {
    if (this.debugMode) {
      switch (method) {
        case 'update rows':
