...
export namespace PixiUtils {
  ...
  export function getAngleTo(mx: number, my: number, px: number, py: number) {
    // var self = that;
    const distX = my - py;
    const distY = mx - px;
  ...
  export function getAngleX(length: number, angle: number) {
    return Math.cos(angle) * length;
  }

  export function getAngleY(length: number, angle: number) {
